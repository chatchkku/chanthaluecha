...
# --- 5. TOP FEATURE: EXACT SPAN EMA WATCHLIST RADAR (OVERSOLD RSI <= 35 FILTER) ---
st.markdown("#### 🔔 Multi-EMA Watchlist Radar (EMA 35, 89, 200 + Oversold RSI $\le$ 35)")
            ...
            if not df_d.empty and len(df_d) >= 30:
                ...
                for span in ema_spans:
                    if len(df_d) >= span:
                        ...
                        # เงื่อนไขกรอง: ใกล้เส้น EMA และ RSI อยู่ในโซน Oversold (<= 35)
...
with st.spinner("กำลังสแกนหุ้นเข้าใกล้เส้น EMA และกรองหาโซน Oversold (RSI <= 35)..."):
...
if multi_radar_results:
    ...
    for item in multi_radar_results:
        ...
        items_html += f"""
            <div class='ticker-item'>
                ...
                RSI: <span style='color:#10b981; font-weight:700;'>{item['rsi']:.1f}</span>
            </div>
        """
    ...
    st.info("ℹ️ ขณะนี้ยังไม่มีหุ้นใน Watchlist ที่ย่อตัวใกล้เส้น EMA พร้อมกับอยู่ในโซน Oversold (RSI <= 35)")

st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)

# --- 6. SMART SEARCH PANEL ---
...
# --- 7. HELPER FUNCTIONS & SMART RESOLVER ---
